...
The amygdala's involvement seems logical given its role in processing fear, with studies demonstrating connections between stereotype threat and anxiety [@abramsAgeApartEffects2006a; @luAnxietyMindWandering2015; @maloneyMathematicsAnxietyStereotype2013]. 
The PFC's importance in cognitive functions, social behaviour, and working memory relates to all hypotheses in this review. 
The DMN's activation is expected due to its relationship with introspection, self-referential thoughts, and mind wandering [@mittnerNeuralModelMind2017; @poerioRoleDefaultMode2017], which have been linked to stereotype threat [e.g., @jordanoStereotypeThreatTrigger2017a; @mrazekThreatenedDistractionMindwandering2011a]. 
...
It should be acknowledged that artificial intelligence has been used as an aid in this review — namely, Anthropic's Claude 3.5 Sonnet [@anthropicClaudeAiSonnet2024], ChatGPT  and GitHub's Copilot [@githubGitHubCopilot2024], the latter was directly integrated into RStudio Server [@positteamRStudioIntegratedDevelopment2024]. The chats that directly influenced this review are all available on the GitHub repository.
...
h1_table[6, ] <- c(
  ...
  "Effects of stereotype threat on neural activation patterns and maths performance; fMRI (vACC, BA47, BA40, BA39)",
  ...
  "Mixed-model ANOVA",
  paste0(
    "Heightened activity in the vACC (BA 32/10) during the second test. $\\textit{F}$(1,26) = ", apa_num(11.41), "***, $\\eta^{2}_\\text{p}$ = ", apa_num(.31), "."
  ),
  "Partially"
)
...
h1_table[8, ] <- c(
  ...
  "Effects of maths stereotype threat on brain region connectivity; rs-fMRI (hippocampus, MCG, cerebellum, PCG)",
  "IV: Test (pre- vs post-test), Condition (stereotype threat vs. control); DV: RSDC of brain regions",
  "Mixed-effect analysis",
  paste0(
    "Significant main effects were found for the hippocampus, MCG, right cerebellum, and left PCG. $\\textit{F}$(1,45) = ", apa_num(4.883), "*."
  ),
  "Partially"
)
...
h2_table[4, ] <- c(
  ...
  "Experimental",
  "28 female undergraduates",
  "Effects of stereotype threat on neural activation and maths performance; fMRI (vACC, DLPFC, IFG, BA47, BA40)",
  "IV: Condition (stereotype threat vs. control), Time of measurements; DV: Neural activation patterns, Maths performance",
  "Mixed-model ANOVA",
  paste0("Increased vACC activation, decreased cognitive region activation under threat; $\\textit{F}$(1,26) = ", apa_num(11.41), "**, $\\eta^{2}_\\text{p}$ = ", apa_p(.31)),
  "Yes"
)
