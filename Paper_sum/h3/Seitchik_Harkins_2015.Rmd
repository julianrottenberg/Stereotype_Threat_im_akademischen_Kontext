...
- Only included Experiment 3 since Experiment 1, 2 and 4 did not explicitly measure working memory.  
...
- Stereotype threat mere effort account
- Working memory account vs mere effort account  
...
This research was funded by the U.S. Army Research Institute for the Behavioral and Social Sciences (Contract W5J9CQ-12-C-0046; PI: Stephen G. Harkins).
...
- In Experiment 3, we directly pitted the mere effort account against the working memory account on horizontal modular problems.  

### What is/are the population focus/foci of the study?

- females under maths stereotype threat  
...
- Working memory account would predict that, on horizontal problems, requiring participants to enter their answers from right-to-left could reduce the debilitation produced by ST. Participants do not need to use the phonological loop for remembering the intermediate value. They can simply perform the unit's subtraction and enter the value in the right hand box. As a result, the fact that the phonological loop is occupied by concerns about fulfilling the threat should have diminished or no impact on performance. However, this shortcut is not available when answers are entered from left-to-right. Because the whole answer is entered, the phonological loop is needed to keep track of the intermediate values. According to Beilock et al.'s account, thoughts about fulfilling the stereotype should interfere with remembering these values, resulting in debilitated performance.  
- In contrast, the mere effort account would predict that ST would potentiate the prepotent response, the method of adjustment. Entering the answer from left-to-right is consistent with the method of adjustment, which produces the whole answer. Thus, the fact that the threat participants are motivated to want to perform well should lead them to outperform no threat participants on horizontal problems when they enter the answers from left-to-right. On the other hand, using the method of adjusted should debilitate performance when the answer is entered from right-to-left. One must remember the adjustments to reach the final answer and then enter this answer from right-to-left, even though it would usually be written from left-to-right. Also, the fact that no threat participants are more likely than threat participants to use the traditional method confers the advantage described above when the digits must be entered from right-to-left (i.e., they do not need to remember the intermediate value and can enter it into the unit's box, making problem solution easier). Thus, the potentiated use of the method of adjustment should lead the threatened participants to perform more poorly than no threat participants when they enter the answers from right-to-left.  
...
- horizontal and vertical subtraction problems
- direction of entry (left-to-right vs. right-to-left)
- manipulation check
...
- 2 (ST vs no ST) x 2 (orientation: horizontal vs vertical) x 2 (entry direction: left-to-right vs right-to-left)  
...
- Procedures identical to Experiment 2 with one exception
- Participants were instructed to enter their answers from left-to-right or from right-to-left 
1. practice problems
2. ST manipulation
3. maths task
4. manipulation check
5. questionnaire about taken approaches
...
- One hundred and sixteen (N=116) female undergraduates from a northeastern university participated in the study for course credit.  
...
- Horizontal or vertical maths subtraction problems
- ST manipulation as used before (e.g. Brown & Pinel, 2003)
- ST manipulation check: "To what extent are there gender differences in performance on this task?" and "Who do you believe performs better on this task?", each on a 11-point scale
- questionnaire about taken approaches
...
- All of the results were analysed in three-way ANOVAs with threat (ST vs. no ST), orientation (horizontal vs. vertical), and entry direction (left-to-right vs. right-to-left) as between-subjects factors as independent variables  
*task performance:*
- to decompose the three-way interaction, two two-way ANOVAs were conducted, one for horizontal problems and one for vertical problems, using the overall error term of the three-way interaction for each analysis.  
...
*Manipulation check:*
- Main effect for ST condition were found for both questions but no other reliable effects were found
- Participants subject to ST reported that gender differences existed to a greater extent than no ST participants
- ST participants also reported that males perform better on the task than females to a greater extent than no ST participants
- These resulsts indicated that the ST manipulation was successful.  

*Task performance:*
- There were no reliable effects for percentage correct
- On problems correct, results revealed a three-way interaction
- Analysis of the horizontal problems in a 2 (threat vs no threat) x 2 (left-to-right vs right-to-left) ANOVA revealed only a Threat x Direction interaction
- Threatened participants solved more horizontal problems correctly than non-threatened participants when the problems were entered from left-to-right
- Non-threatened participants solved more problems correctly than threatened participants when they were entered from right-to-left
- Analysis of the vertical problems in a 2 (ST vs no ST) x 2 (left-to-right vs right-to-left) ANOVA revealed only a main effect for threat
- Participants subject to ST solved more vertical problems correctly than participants who were not subject to threat.  

*Solution approach measure*:  
- This analysis revealed three main effects
- Threatened participants reported using the traditional method on a smaller proportion than non-threatened participants
- Participants working on horizontal problems reported using the traditional method on a smaller proportion than participants working on vertical problems
- Participants reported using the traditional method on a smaller proportion of problems entered from left-to-right than from right-to-left.   
...
On horizontal problems, we found that threatened participants performed better than controls when answers were entered from left-to-right, but worse when the answeres were entered from right-to-left. This pattern of findings is consistent with mere effort, but not the working memory account.  
lthough these findings support the mere effort account, but not the working memory explanation, for performance debilitation on horizontal subtraction problems, they do not explain why threatened participants perform more poorly than non-threatened participants on Beilock et al.'s (2007) horizontal modular arithmetic problems. In fact, the designs of both Experiments 2 and 3 incorporate features that permit tests of hypotheses that are relevant to performance on horizontal subtraction problems but are not present in horizontal modular arithmetic problems.  
...
- no mention of ehtics approval or consent or debriefing  
...
- the study found different results than previous studies
- the sample was not described in detail
...
- somehow Harkins and Jamieson seem to be the only people that can produce these findings. Other researchers either are able to confirm the working memory account (to be fair, here it's mainly Schmader and Forbes or Johns but also a few others) or neither of the two accounts.  
Also previous research mentioned that they do not agree with the conclusions drawn by Harkins and Jamieson - if this is applicable to this study as well is hard to tell.
