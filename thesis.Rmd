...
This second screening was done by looking into the full-text of each paper, here another 44 papers were excluded for the following reasons: wrong focus ($n = 29$), wrong population ($n = 7$), wrong study design ($n = 5$), wrong publication type ($n = 2$) - an overview of this can be found in the PRISMA flowchart [@haddawayPRISMA2020PackageShiny2022] in Figure \ref{fig:prisma}.
...
An ANOVA on performance, using time (Time 1 vs. Time 2) as a within-subjects variables and condition (threat vs. control) as a between-subjects variable, revealed a significant interaction, *F*(1,26) = `r apa_num(11.41)`, *p* < `r apa_p(.005)`, $\eta^{2}_{p}$ = `r apa_num(.31)`, however, no main effect of either time or condition.
...
Using a mixed-model ANOVA on reaction times to analyse performance, a main effect of time, *F*(1,26) = `r apa_num(6.21)`, *p* < `r apa_p(.02)`, $\eta^{2}_{p}$ = `r apa_num(.19)`, but not condition, $F < 1$, was found, neither was an interaction, *F*(1,26) = `r apa_num(1.21)`, *p* = `r apa_p(.28)`.
...
Using a 2 (Gender) $\times$ 2 (Condition) ANOVA on each executive function task, inhibition showed a significant two-way interaction, *F*(1,164) = `r apa_num(5.73)`, *p* = `r apa_p(.018)`, $\eta^{2}_{\text{p}}$ = `r apa_num(.034)`), with only womens' inhibition suffering under threat, *F*(1,164) = `r apa_num(7.95)`, *p* = `r apa_p(.005)`, $\eta^{2}_{\text{p}}$ = `r apa_num(.046)`).
The same pattern was found for updating, *F*(1,164) = `r apa_num(15.24)`, *p* < `r apa_p(.001)`, $\eta^{2}_{\text{p}}$ = `r apa_num(.085)`), and *F*(1,164) = `r apa_num(20.89)`, *p* < `r apa_p(.001)`, $\eta^{2}_{\text{p}}$ = `r apa_num(.113)`, respectively.
Shifting showed no significant results.
However, analysing accuracy and reaction time, for the maths test, to a 2 (Gender) $\times$ 2 (Condition) ANOVA, the same two-way interaction, *F*(1,164) = `r apa_num(15.95)`, *p* < `r apa_p(.001)`, $\eta^{2}_{\text{p}}$ = `r apa_num(.089)` and performance decrease under threat was found, for accuracy, *F*(1,164) = `r apa_num(20.22)`, *p* < `r apa_p(.001)`, $\eta^{2}_{\text{p}}$ = `r apa_num(.110)`, while reaction time reaching a marginally significant main effect of gender, *F*(1,164) = `r apa_num(3.59)`, *p* = `r apa_p(.060)`, $\eta^{2}_{\text{p}}$ = `r apa_num(.21)`.
...
ANOVAs showed a significant effect of stereotype threat on Stroop inference, *F*(1,61) = `r apa_num(8.69)`, *p* = `r apa_p(.004)`, $\eta^{2}_{\text{p}}$ = `r apa_num(.13)`, this effect was not due to a speed-accuracy trade-off.  
...
A main effect of threat was found, *F*(1,102) = `r apa_num(8.91)`, *p* = `r apa_p(.004)`, $\eta^{2}_{\text{p}}$ = `r apa_num(.08)`, using a 2 (condition) $\times$ 3 (regulatory focus) ANOVA.
Further, Experiment 1's results for the Stroop task, were replicated,*F*(1, 101) = `r apa_num(2.80)`, *p* < `r apa_num(.10)`, $\eta^{2}_{\text{p}}$ = `r apa_num(.03)`, and an interaction on Stroop inferences using a 2 $\times$ 3 ANOVA were found, *F*(2,101) = `r apa_num(3.07)`, *p* = `r apa_p(.05)`, $\eta^{2}_{\text{p}}$ = `r apa_num(.06)`).
The Stroop inference was only significant under threat, in the prevention condition, *F*(1,101) = `r apa_num(3.6)`, *p* = `r apa_p(.24)`, $\eta^{2}_{\text{p}}$ = `r apa_num(.01)`.  
...
Here, the 2 (condition) $\times$ 2 (regulatory focus) $\times$ 2 (maths task order) interaction was found to be significant on maths performance (% of correct responses), *F*(1,150) = `r apa_num(13.30)`, *p* < `r apa_p(.001)`, $\eta^{2}_{\text{p}}$ = `r apa_num(.08)`.
...
For maths performance, measured as response times, the same three-way interaction was significant, *F*(1,152) = `r apa_num(4.69)`, *p* = `r apa_p(.03)`, $\eta^{2}_{\text{p}}$ = `r apa_num(.03)`.
...
In the maths last condition, the two-way interaction was marginally significant, *F*(1,152) = `r apa_num(3.11)`, *p* = `r apa_p(.08)`, $\eta^{2}_{\text{p}}$ = `r apa_num(.02)`).
...
Moreover, no significant effect was found for switching, neither for gender, nor for threat, for cluster size, only a significant effect of gender was found, *F*(1,159) = `r apa_num(4.123)`, *p* < `r apa_p(0.05)`, $\eta^{2}_{\text{p}}$ = `r apa_num(0.025)`, with males scoring higher than females.
...
In the Stereotype Threat/SART Related condition, commission errors were fewer (*M* = `r apa_num(4.89(`, *SD* = `r apa_num(3.03)`), compared to Stereotype Threat/SART Unrelated (*M* = `r apa_num(10.44)`, *SD* = `r apa_num(4.55)`), *F*(1, 69) = `r apa_num(28.78)`, *p* < `r apa_p(.001)`, $\eta^{2}_{\text{p}}$ `r apa_num(.294)`, omissions did not differ significantly between these conditions, *F*(1, 69) = `r apa_num(4.91)`, *p* = `r apa_p(.030)`, $\eta^{2}_{\text{p}}$ = `r apa_num(.066)`, anticipations were lower in the Stereotype Threat/SART Related condition (*M* = `r apa_num(0.56)`, *SD* = `r apa_num(1.10)`), compared to Stereotype Threat/SART Unrelated (*M* = `r apa_num(4.44)`, *SD* = `r apa_num(7.09)`), *F*(1, 69) = `r apa_num(11.42)`, *p* < `r apa_p(.01)`, $\eta^{2}_{\text{p}}$ = `r apa_num(.142)`, and RT coefficient of variation showed less variablility in the Stereotype Threat/SART Releated condition (*M* = `r apa_num(258)`, *SD* = `r apa_num(76)`), compared to Stereotype Threat/SART Unrelated (*M* = `r apa_num(341)`, *SD* = `r apa_num(97)`), *F*(1, 69) = `r apa_num(11.75)`, *p* < `r apa_p(.001)`, $\eta^{2}_{\text{p}}$ = `r apa_num(.146)`. 
While a significant effect for the mere effort contrast was found, Wilk's $\Lambda$ = `r apa_num(.657)`, *F*(4, 66) = `r apa_num(8.63)`, *p* < `r apa_p(.001)`, $\eta^{2}_{\text{p}}$ = `r apa_num(.343)`, the counter-hypothesis was not supported, Wilk's $\Lambda$ = `r apa_num(.993)`, *F*(4, 66) = `r apa_num(1.15)`, *p* = `r apa_p(.33)`, $\eta^{2}_{\text{p}}$ = `r apa_num(.065)`.  
