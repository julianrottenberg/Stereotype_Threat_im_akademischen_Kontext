---
title             : "Stereotype threat im akademischen Context"
shorttitle        : " "

author: 
  - name          : "Julian Rottenberg"
    affiliation   : "1"
...
affiliation:
  - id            : "1"
    institution   : "Fakultät für Psychologie der FernUniversität in Hagen"
...
abstract: |
  Stereotype threat, a phenomenon where awareness of negative stereotypes impairs performance, has been extensively studied in academic contexts. However, its underlying neural and cognitive mechanisms have received less attention. This systematic review uses social neuroscience to examine how stereotype threat affects brain activation, cognitive control, and working memory in academic settings. The analysis of 28 studies revealed complex patterns of neural activation under threat, with increased activity in emotional processing regions and decreased activity in cognitive control areas. Cognitive control, particularly inhibition and updating, was often impaired. Working memory capacity consistently decreased under stereotype threat. These findings support an integrated model where stereotype threat induces affective and cognitive responses that collectively tax working memory resources. This review highlights the need for interventions targeting both emotional regulation and cognitive support to mitigate stereotype threat effects in academic environments.
  
  **Zusammenfassung**: 
  Stereotype threat, ein Phänomen, bei dem das Bewusstsein für negative Stereotype die Leistung beeinträchtigt, wurde im akademischen Kontext umfassend untersucht. Die zugrundeliegenden neuralen und kognitiven Mechanismen erhielten jedoch weniger Aufmerksamkeit. Dieses systematische Review nutzt die soziale Neurowissenschaft, um die Auswirkungen auf Hirnaktivierung, kognitive Kontrolle und Arbeitsgedächtnis zu untersuchen. Die Auswertung von 28 Studien zeigte komplexe Muster neuronaler Aktivierung, mit erhöhter Aktivität in Regionen der emotionalen Verarbeitung und verringerter Aktivität in Bereichen der kognitiven Kontrolle. Kognitive Kontrolle, insbesondere Hemmung und Aktualisierung, war oft beeinträchtigt. Die Arbeitsgedächtniskapazität nahm durchgängig ab. Diese Ergebnisse stützen ein integriertes Modell, bei dem Stereotype threat affektive und kognitive Reaktionen hervorruft, die gemeinsam die Arbeitsgedächtnisressourcen belasten. Das Review betont die Notwendigkeit von Interventionen, die sowohl emotionale Regulation als auch kognitive Unterstützung adressieren, um die Auswirkungen von Stereotype threat im akademischen Umfeld zu mildern.
...
header-includes:
  - |
    ...
    \thispagestyle{fancy}
    \duedate{19.07.2024}
    \course{Bachelorarbeit im Studiengang \\ B. Sc. Psychologie \\ Lehrgebiet Psychologische Methodenlehre und Evaluation} 
    \professor{Erstprüfer/in:  \\ Zweitprüfer/in: }
...
# Ensure single spacing is available
knitr::knit_hooks$set(singlespace = function(before, options, envir) {
  if (before) {
...
# Read the CSV file
abbreviations <- read.csv("files/abbre.csv", stringsAsFactors = FALSE)

# Sort the abbreviations alphabetically
abbreviations <- abbreviations %>% arrange(Abbreviation)

# Calculate the total number of rows
total_rows <- nrow(abbreviations)

# Calculate the midpoint to split the list
midpoint <- ceiling((total_rows + 4) / 2)

# Function to generate LaTeX rows for abbreviations
generate_latex_rows <- function(abbr_data) {
  latex_rows <- sapply(1:nrow(abbr_data), function(i) {
    sprintf("\\textbf{%s} & %s \\\\", 
            abbr_data$Abbreviation[i], 
            abbr_data$Definition[i])
  })
  paste(latex_rows, collapse = "\n")
}

# Generate the LaTeX content for left and right columns
left_content <- generate_latex_rows(abbreviations[1:midpoint,])
right_content <- generate_latex_rows(abbreviations[(midpoint+1):nrow(abbreviations),])

# Write to files
writeLines(left_content, "abbreviations_left.tex")
writeLines(right_content, "abbreviations_right.tex")
```
...
\hyperref[tab:neuro_table]{\hbox to \textwidth{Table 1. Overview of the EEG and fMRI measurements \dotfill 18}}

\hyperref[tab:query_table]{\hbox to \textwidth{Table 2. Search queries used for the systematic literature review. \dotfill 24}}

\hyperref[fig:prisma]{\hbox to \textwidth{Figure 1. PRISMA Flow Diagram of Study Selection Process \dotfill 26}}

\hyperref[tab:h1_table]{\hbox to \textwidth{Table 3. Overview of the Included Papers for Hypothesis 1 \dotfill 34}}

\hyperref[tab:h2_table]{\hbox to \textwidth{Table 4. Overview of the Included Papers for Hypothesis 2 \dotfill 44}}

\hyperref[tab:h3_table]{\hbox to \textwidth{Table 5. Overview of the Included Papers for Hypothesis 3 \dotfill 57}}




% Add more tables and figures as needed, following the same format
\end{flushleft}
...
\vspace{1ex}
\begin{tabular}{@{}p{4em}p{0.75\textwidth}@{}}
\input{abbreviations_left.tex}
\end{tabular}
\end{minipage}
\hfill
\begin{minipage}[t]{0.45\textwidth}
...
Its main subregions include the dorsolateral prefrontal cortex (dlPFC), orbitofrontal cortex, ventrolateral prefrontal cortex (vlPFC), ventromedial prefrontal cortex (vmPFC), anterior cingulate cortex (ACC), and frontal pole [@chafeePrefrontalCortex2022; @vandenbosPrefrontalCortex2015].
The dlPFC is important for working memory and cognitive flexibility, the orbitofrontal cortex evaluates rewards and punishments, crucial for decision-making, the vlPFC aids in inhibitory control and attention while the vmPFC regulates emotions and value-based decisions [@chafeePrefrontalCortex2022; @tefferHumanPrefrontalCortex2012; @eysenckConsciousness2020].
...
It plays a critical role in switching between the DMN and the central executive network, enabling the brain to transition from a resting state to an active, task-focused state [@gouldenSalienceNetworkResponsible2014; @rensProactiveRecruitmentFrontoparietal2017; @schimmelpfennigRoleSalienceNetwork2023]
...
This process is known as the blood-oxygen-level dependent contrast, which is detected by the fMRI to produce images showing the active parts of the brain [@barberStereotypeThreatCan2013; @matherHowFMRICan2013].
...
This technique identifies functional connectivity between different brain regions by analysing spontaneous fluctuations in the blood-oxygen-level dependent signal — the DMN, for example, is studied using rs-fMRI [@matherHowFMRICan2013; @eysenckCognitiveNeuroscienceBrain2020].  
...
The lateral prefrontal cortex plays a crucial role in cognitive control by holding information in working memory and guiding behaviour based on current goals [@cohenCognitiveControlCore2017; @mackieCognitiveControlAttentional2013].
...
apa_table(query_table, 
          ...
          note = "\\small The search queries were used in the databases Web of Science, Google Scholar, PSYNDEX, ResearchRabbit, and EBSCOhost. The permalinks to each search used can be found within the GitHub repository.",
          escape = FALSE,
          font_size = "small",
...
The template was a mixture of the following checklists: Critical Appraisal Skills Programme systematic review checklist [@criticalappraisalskillsprogrammeCASPSystematicReview2018], Review guidelines for extracting data and quality assessing primary studies in educational research [@eppi-centreReviewGuidelinesExtracting2003], Critical appraisal checklist for a systematic review [@universityofglasgowCriticalAppraisalChecklistn.d.nodate], and the Newcastle-Ottawa scale for assessing the quality of nonrandomised studies in meta-analyses [@wellsNewcastleottawaScaleNOS2014], which are used to describe studies and assess their quality.
...
```{r prisma, fig.cap = "PRISMA Flow Diagram of Study Selection Process"}
...
Experiment 3A: For the modular arithemetic (MA) problems, a three-way interaction between the independent variables was found, *F*(1,31) = `r apa_num(4.12)`, *p* = `r apa_p(.05)`, $\eta^{2}_{p}$ = `r apa_p(.12)`.
...
The task-related power changes revealed a main effect for Stereotype Exposure (*F*(1, 54) = `r apa_num(3.93)`, *p* = `r apa_p(.05)`, $\text{partial }\eta^{2}$ = `r apa_num(.07)`), with heightened cortical activation (*M* = `r apa_num(0.07)`, *SD* = `r apa_num(0.03)`) in the stereotype threat condition. 
...
Here, the mean RSDC *z*-values were only higher in the stereotype threat condition in the right superior parietal gyrus, left precuneus, left middle occipital gyrus, and right angular gyrus.
...
h1_table[1, ] <- c(
  ...
  "Effects of stereotype threat on working memory and its influence on unrelated tasks; behavioural tasks (phonological loop, central executive network)",
...
h1_table[5, ] <- c(
  "Jończyk et al. (2022)",
  "Experimental",
  "23 female undergraduates in the United States",
...
h1_table[8, ] <- c(
  ...
  paste0(
    "Significant main effects were found for the hippocampus, MCG, right cerebellum, and left precentral gyrus; left precuneus: $\\textit{F}$(1,45) = ", apa_num(8.426), "**."
  ),
  "Partially"
)
...
The Mental-Rotation-Task-3D showed a significant main effect of Sex, with men performing better than women, *F*(1,128) = `r apa_num(10.97)`, *p* = `r apa_p(.001)`, $\eta^{2}$ = `r apa_num(.08)`, *d* = `r apa_num(0.57)`, while Condition showed a significant main effect in the Mirror-Pictures-2D, *F*(1,128) = `r apa_num(4.70)`, *p* = `r apa_p(.032)`, $\eta^{2}$ = `r apa_num(.04)`.
For verbal fluency, interactions were found between Condition and Group Sex Composition, in both, the Word Fluency, *F*(1,128) = `r apa_num(4.49)`, *p* = `r apa_p(.036)`, $\eta^{2}$ = `r apa_num(.03)`,and the 4W, *F*(1,128) = `r apa_num(6.30)`, *p* = `r apa_p(.013)`, $\eta^{2}$ = `r apa_num(.05)`).
...
Perceptual speed also showed the significant Condition and Group Sex Composition interaction, *F*(1,128) = `r apa_num(6.89)`, *p* = `r apa_p(.009)`, $\eta^{2}$ = `r apa_num(.05)`.
Condition, again showed a significant main effect, this time in the perceptual speed, *F*(1,128) = `r apa_num(12.65)`, *p* = `r apa_p(.001)`, $\eta^{2}$ = `r apa_num(.09)`.
...
In the first experiment, it was found that participants under stereotype threat reported significantly more task-related interference compared to the control group, *F*(1,58) = `r apa_num(5.669)`, *p* = `r apa_p(0.021)`, *d* = `r apa_num(0.636)`.  
Neither maths verification accuracy nor letter recall accuracy were worse under stereotype threat.
In the second experiment, the same pattern was found, with participants under stereotype threat reporting significantly more task related inference, *F*(1,58) = `r apa_num(5.527)`, *p* = `r apa_p(0.022)`, *d* = `r apa_num(0.423)`.
...
h2_table[8, ] <- c(
  ...
  "Effects of menstruation stereotype threat on cognitive performance; Stroop test, Scholastic Assessment Test-like maths test",
...
For the anti-saccade task, neither saccade reaction time for correct saccades, nor their accuracy revealed a significant main effect of stereotype threat, *F*(2, 58) = `r apa_num(0.30)`, *p* = `r apa_p(0.75)`, $\eta^{2}_{\text{p}}$ = `r apa_p(0.01)`), and *F*(2, 57) = `r apa_num(0.03)`, *p* = `r apa_p(0.97)`, $\eta^{2}_{\text{p}}$ < `r apa_p(0.001)`, respectively.
The same can be said for reflexive saccades and their saccade reaction times, *F*(2, 57) = `r apa_num(0.03)`, *p* = `r apa_p(0.97)`, $\eta^{2}_{\text{p}}$ < `r apa_p(0.001)`, and *F*(2, 56) = `r apa_num(0.25)`, *p* = `r apa_p(0.78)`, $\eta^{2}_{\text{p}}$ = `r apa_p(0.009)`, respectively.
While the saccade reaction times for corrective saccades, again, did not reach significance for stereotype threat, *F*(2, 53) = `r apa_num(0.3)`, *p* = `r apa_p(.75)`, $\eta^{2}_{\text{p}}$ = `r apa_p(0.01)`, the percentage of corrective saccades did, *F*(2, 57) = `r apa_num(3.57)`, *p* = `r apa_p(.0035)`, $\eta^{2}_{\text{p}}$ = `r apa_p(0.11)`.  
...
h3_table[3, ] <- c(
  ...
  "Experimental",
  "Experiment 1: 31 female college students; Experiment 3A: 33 female college students; Experiment 4: 30 female college students; Experiment 5: 33 female college students",
  "Effects of stereotype threat on working memory and its influence on unrelated tasks; MA (processing speed); n-back task (capacity, accuracy)",
  "IV: Group (stereotype threat vs. control), Problem working memory demand (low vs. high), Block (baseline vs. posttest); DV: Accuracy, Reaction time",
  "ANOVA",
  paste0(
    "High-demand problems showed a significant decrease in accuracy at the post-test, CI [", apa_num(81), "\\% - ", apa_num(97), "\\%]; $\\textit{d}$ = ", apa_num(.61), ". $\\textit{F}$(1,29) = ", apa_num(11.18), ", $\\eta^{2}_\\text{p}$ = ", apa_num(.28), "."
...
h3_table[6, ] <- c(
  ...
  "Regression analysis",
  paste0(
    "Stroop effect larger under threat for low working memory capacity individuals. $\\beta$ = ", apa_num(0.12), ", $\\beta$ = -", apa_num(0.11), ", $\\beta$ = ", apa_num(0.24), "*."
  ),
  "Partially"
)
...
@beilockStereotypeThreatWorking2007 is difficult to interpret as no neuroimaging techniques were used. 
...
For instance, @forbesSpontaneousDefaultMode2015 observed increased phase-locking in the DMN under stereotype threat, suggesting heightened self-referential thinking and emotional processing. 
...
However, contradictory results were also found, such as heightened activation of the cerebral cortex, cerebellar cortex [@dunstSexDifferencesNeural2013], or vACC alongside Brodmann areas 11 and 2.
...
@rydellStereotypeThreatExecutive2014, alongside @beilockStereotypeThreatWorking2007, demonstrated that executive function is impaired under stereotype threat, and they were also able to identify updating as the supporting force.
@jordanoPrimingPerformancerelatedConcerns2017 provided another approach, showing that stereotype threat increased mind-wandering, which could impair cognitive control during tasks requiring sustained attention.
...
@beilockStereotypeThreatWorking2007 demonstrated that stereotype threat significantly hampers performance on tasks requiring substantial working memory resources, particularly those reliant on phonological aspects of the system. 
...
Similarly, @schmaderConvergingEvidenceThat2003 found that stereotype threat reduces working memory capacity, specifically that the supplemental information exceeds the given capacity.
...
The integrated process model proposed by @schmaderIntegratedProcessModel2008 offers a comprehensive explanation by suggesting that stereotype threat induces a combination of negative affective, motivational, and physiological responses that collectively place demands on the executive-control components of working memory.
However, it also needs to be mentioned that not all studies came to this conclusion. 
While @jamiesonMereEffortStereotype2007 and @brownThreatDoesNot2016 argued that the mere effort account best describes their results, which were improvements in performance under threat if certain conditions are given, @penningtonStereotypeThreatMay2019 deemed neither the working memory interference nor the mere effort account an appropriate explanation.

## Integration with Existing Research

The repeated finding of heightened affective processing, whether through the DMN, SN, or related areas, aligns with behavioural observations from past research, which have noted increased worry and fear in individuals under stereotype threat [e.g., @bouazzaouiMemoryAgebasedStereotype2020; @osborneLinkingStereotypeThreat2007; @zhangRoleStereotypeThreat2023; @stephanWomenAttitudesMen2000].
While not in line with findings by @brownThreatDoesNot2016, the found impairment of cognitive control seems to mirror the majority of the existing research on it [e.g., @caughieAgebasedStereotypeThreat2023a; @fawsittExtendingStereotypeEmbodiment2017; @schusterDistractedUnthoughtSuppression2015; @steeleStereotypeThreatIntellectual1995; @croizetStereotypeThreatUndermines2004].

Regarding Hypothesis 3, there is some overlap with H2. Additionally, it also seems to correspond with existing findings [e.g. @nahidiEffectGenderStereotype2023; @piroelleNewApproachInvestigate2022; @songInferringQualityPrice2023].
As mentioned before, @jamiesonMereEffortStereotype2007's mere effort account finds different results and draws other conclusions. 
...
@beasleyWhyTheyLeave2012 mentioned this in relation to women in Science, Technology, Engineering, and Mathematics, pointing out that those who made it to college might have found coping mechanisms or adjusted to the stereotype threat. As @dumitruInvestigatingGenderDifferences2022a pointed out, when investigating women in Science, Technology, Engineering, and Mathematics, they might already have adjusted or found compensatory mechanisms to deal with stereotype threat, since they need and want to overcome it to pursue their area of interest.
@jonczykHowStereotypeThreat2022 state in their discussion that their sample might be skewed as students struggling with their studies might not have participated in the study as they do not have the time to spare. 
...
@spencerStereotypeThreatWomen1999 found that stereotype threat undermines performance most on challenging tasks. 
...
@stahlRolePreventionFocus2012 made it a point to consider participants' major and used this in their stereotype threat manipulation. While other studies showed that the age ranges between 'undergraduates' varied greatly.
Additionally, if ethnicity-based stereotype threat was not a direct focus, it mostly was not mentioned, or in the cases where it was [e.g., @penningtonStereotypeThreatMay2019], it was neither further investigated nor was it mentioned in enough detail to draw any conclusions from it. 
...
However, other researchers were unable to replicate these findings or agree with the interpretations [cf. @penningtonTwentyYearsStereotype2016].
...
Among others, @bauerEffectMindfulnessStereotype2021 and @raheDoesMindfulnessHelp2023a have already successfully tested mindfulness — a cost-efficient and individually realizable intervention — to decrease stereotype threat effects.   
